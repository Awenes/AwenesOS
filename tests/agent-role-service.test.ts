...
describe("agent role service", () => {
  it("seeds the four provider-neutral built-in roles idempotently", async () => {
    const opened = await openDatabase(":memory:"); const repository = new AgentRoleRepository(opened.db); const service = new AgentRoleService(repository, new ProjectRepository(opened.db));
  ...
  it("creates project roles and records model and enabled-state changes", async () => {
    const opened = await openDatabase(":memory:"); const projects = new ProjectRepository(opened.db); const repository = new AgentRoleRepository(opened.db); const service = new AgentRoleService(repository, projects);
    ...
    const assigned = await service.assignModel(role.id, "anthropic", "claude-sonnet"); expect(assigned).toMatchObject({ providerId: "anthropic", modelId: "claude-sonnet" });
  ...
  it("rejects a role that assigns only a provider or only a model", async () => {
    const opened = await openDatabase(":memory:"); const service = new AgentRoleService(new AgentRoleRepository(opened.db), new ProjectRepository(opened.db));
