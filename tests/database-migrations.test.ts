...
describe("database migrations", () => {
  it("creates and records the current schema for a new database", async () => {
    ...
    expect(opened.migration).toMatchObject({ fromVersion: 0, toVersion: 5, applied: [1, 2, 3, 4, 5], backupPath: null });
    expect(await inspectDatabase(opened.client)).toEqual({ healthy: true, integrity: "ok", foreignKeyViolations: 0, databaseVersion: 5, currentVersion: 5 });
  ...
  it("backs up and adopts a legacy database without losing task rows", async () => {
    ...
    const opened = await openDatabase(path);
    expect(opened.migration.applied).toEqual([1, 2, 3, 4, 5]); expect(opened.migration.backupPath).toBeTruthy(); expect((await stat(opened.migration.backupPath!)).isFile()).toBe(true);
  ...
  it("does not create another backup when no migration is pending", async () => {
    ...
    expect(second.migration).toMatchObject({ fromVersion: 5, toVersion: 5, applied: [], backupPath: null }); await second.client.close();
  });
});
