...
} from "../src/domain/git-evidence.js";
...
async function setup(adapter?: CrmTaskAdapter) {
  const dir = await mkdtemp(join(tmpdir(), "awenes-"));
  dirs.push(dir);
  ...
  return {
    service: new TaskService(
      repository,
      adapter ?? new MockCrmAdapter(join(dir, "crm.json")),
    ),
    repository,
...
describe("first vertical slice", () => {
  it("tracks work locally and requires explicit manual CRM completion confirmation", async () => {
    const dir = await mkdtemp(join(tmpdir(), "awenes-manual-"));
    dirs.push(dir);
    const opened = await openDatabase(":memory:");
    const repository = new TaskRepository(opened.db);
    ...
    const task = await service.capture({
      title: "Manual CRM workflow",
      source: "manual",
      assignmentDescription: "Keep external status aligned",
      assignedToMe: false,
      occurredAt: new Date(),
    });
    await service.claim(task.id);
    await expect(service.mapToCrm(task.id, "CRM", undefined)).rejects.toThrow(
      "existing CRM task ID",
    );
    await service.mapToCrm(
      task.id,
      "CRM",
      "TASK-42",
      "https://crm.example.test/tasks/42",
    );

    expect((await service.start(task.id)).status).toBe("in_progress");
    expect(await service.pendingManualCrmUpdates()).toEqual([
    ...
    expect(await service.pendingManualCrmUpdates()).toEqual([
      expect.objectContaining({ desiredStatus: "Paused" }),
    ]);
    ...
    await service.prepareCompletion(
      task.id,
      "Finished and verified the requested work.",
    );
    expect((await service.complete(task.id)).status).toBe("sync_pending");
    expect(await service.pendingManualCrmUpdates()).toEqual([
      expect.objectContaining({
        desiredStatus: "Completed",
        description: "Finished and verified the requested work.",
      }),
    ]);
  ...
  it("captures, claims, maps, executes, completes, and records history", async () => {
    ...
    const task = await service.capture({
      title: "Fix guardian invite flow",
      source: "teams",
      assignmentDescription: "Prevent duplicate invites",
      assignedToMe: false,
      occurredAt: new Date(),
    });
    ...
    await service.editCompletion(
      task.id,
      "Prevented duplicate invites and added regression coverage.",
    );
    const completed = await service.complete(task.id);
    expect(completed.status).toBe("completed");
    expect(
      (await repository.history(task.id)).map((event) => event.type),
    ).toContain("task.crm_sync_succeeded");
    client.close();
  });
  ...
  it("keeps completion pending when external sync fails", async () => {
    ...
    const task = await service.capture({
      ...
      source: "manual",
      assignmentDescription: "Test retry",
      assignedToMe: false,
      occurredAt: new Date(),
    });
    await service.claim(task.id);
  ...
  it("requires explicit confirmation after a manual SharePoint tracker update", async () => {
    const { service, repository, client } = await setup();
    const task = await service.capture({
      title: "Fix mobile filter",
      source: "bug-tracker",
      sourceReference: "sharepoint:agege:mobile:FEATURE-9",
      assignmentDescription: "Remove unused filter",
      assignedToMe: false,
      occurredAt: new Date(),
    });
    await service.claim(task.id);
    ...
    await service.prepareCompletion(
      task.id,
      "Removed the unused mobile filter and verified the screen.",
    );
    ...
    expect(await service.standup()).not.toContain("SharePoint");
    await service.confirmTrackerUpdate(task.id);
    expect(await service.pendingTrackerUpdates()).toHaveLength(0);
    expect(await service.standup()).not.toContain(
      "manual SharePoint tracker update pending",
    );
    expect(
      (await repository.history(task.id)).map((event) => event.type),
    ).toEqual(
      expect.arrayContaining([
        ...
        "task.tracker_manual_update_confirmed",
      ]),
    );
    client.close();
  });
  ...
  it("collects Git metadata since task start without duplicating evidence", async () => {
    const { service, repository, client } = await setup();
    const task = await service.capture({
      title: "Add attendance validation",
      source: "manual",
      assignmentDescription: "Validate attendance input",
      assignedToMe: false,
      occurredAt: new Date(),
    });
    await service.claim(task.id);
    await service.mapToCrm(task.id, "project-1");
    await service.start(task.id);
    const inspector: GitRepositoryInspector = {
      async inspect() {
        return {
          repository: "C:/work/app",
    ...
    let observedRepository: string | undefined;
    const collector: GitEvidenceCollector = {
      async collect(input) {
        observedSince = input.since;
        observedRepository = input.repositoryPath;
        return {
          repository: "C:/work/app",
          branch: "feature/attendance",
          commits: ["abc123\tAdd attendance validation"],
          files: ["src/attendance.ts", "tests/attendance.test.ts"],
        };
      },
    };
    expect(
      await service.collectGitEvidence(task.id, undefined, collector),
    ).toMatchObject({ commits: 1, files: 2, added: 3, skippedExisting: 0 });
    expect(observedSince).toBeInstanceOf(Date);
    expect(observedRepository).toBe("C:/work/app");
    expect(
      await service.collectGitEvidence(task.id, undefined, collector),
    ).toMatchObject({ added: 0, skippedExisting: 3 });
    expect(await repository.evidenceFor(task.id)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "commit",
          value: "abc123\tAdd attendance validation",
        }),
        expect.objectContaining({
          kind: "file",
          value: "C:/work/app: src/attendance.ts",
        }),
      ]),
    );
    client.close();
  ...
  it("records repository remapping instead of silently changing task context", async () => {
    ...
    const task = await service.capture({
      ...
      source: "manual",
      assignmentDescription: "Map repository",
      assignedToMe: false,
      occurredAt: new Date(),
    });
    await service.claim(task.id);
    const first: GitRepositoryInspector = {
      async inspect() {
        return { repository: "C:/work/one", branch: "main", head: "111" };
      },
    };
    const second: GitRepositoryInspector = {
      async inspect() {
        return { repository: "C:/work/two", branch: "feature", head: "222" };
      },
    };
    await service.attachRepository(task.id, ".", first);
    expect(await service.attachRepository(task.id, ".", second)).toMatchObject({
      repositoryRoot: "C:/work/two",
      branchAtMapping: "feature",
      headAtMapping: "222",
    });
    expect(
      (await repository.history(task.id)).map((event) => event.type),
      ...
      expect.arrayContaining([
        "task.repository_mapped",
        "task.repository_remapped",
      ]),
    );
    client.close();
  });
  ...
  it("builds a structured deterministic completion draft from reviewed evidence", async () => {
    ...
    const task = await service.capture({
      title: "Improve enrollment import",
      source: "bug-tracker",
      sourceReference: "AGEGE / General Tests / FEATURE-12",
    ...
    await service.claim(task.id);
    await service.mapToCrm(task.id, "project-1");
    await service.attachRepository(task.id, ".", {
      async inspect() {
        return {
          repository: "C:/work/skoolbod",
    ...
    await service.addEvidence(
      task.id,
      "commit",
      "abcdef123456\tHandle malformed enrollment rows",
    );
    await service.addEvidence(
      task.id,
      "file",
      "C:/work/skoolbod: src/enrollment/import.ts",
    );
    await service.addEvidence(
      task.id,
      "test",
      "Enrollment import regression suite passed",
    );
    await service.addEvidence(task.id, "build", "Production build passed");
    await service.addEvidence(
      task.id,
      "link",
      "https://example.test/result/12",
    );
  ...
  it("flags an evidence-free completion draft for review", async () => {
    ...
    await service.mapToCrm(task.id, "project-1");
    await service.start(task.id);
    expect(await service.draftCompletion(task.id)).toContain(
      "No completion evidence is recorded yet",
    );
    client.close();
  });
});
