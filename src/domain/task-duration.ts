...
export function calculateTaskDuration(events: TimedTaskEvent[], now = new Date()): TaskDuration {
  ...
  for (const event of ordered) {
    if (!["task.started", "task.resumed", "task.paused", "task.completion_prepared", "task.completed", "task.crm_sync_succeeded"].includes(event.type)) continue;
    ...
    if ((event.type === "task.completion_prepared" || event.type === "task.completed" || event.type === "task.crm_sync_succeeded")) {
