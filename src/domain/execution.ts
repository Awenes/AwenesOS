import { z } from "zod";
export const ExecutionRequestSchema=z.object({command:z.string().trim().min(1),args:z.array(z.string()).max(200).default([]),cwd:z.string().trim().min(1),network:z.enum(["none","localhost","public"]).default("none"),stdin:z.string().max(2_000_000).optional()});
