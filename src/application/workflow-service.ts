...
export class WorkflowService {
  ...
  recoverInterrupted(){return this.runs.recoverInterrupted();}
  ...
  async resume(id:string){const run=await this.runs.get(id);if(run.status!=="paused"&&run.status!=="failed")throw new Error(`Cannot resume a ${run.status} run`);const steps=await this.runs.steps(id);const next=steps.find(step=>step.status==="pending"||step.status==="failed");return this.runs.setState(id,"running",next?.stage??"delivery");}
}
