...
export class WorktreeService {
  constructor(private readonly projects: ProjectRepository, private readonly tasks: TaskRepository, private readonly git: GitWorktreeDriver, private readonly initializer?: GitRepositoryInitializer) {}
  ...
  async create(taskId: string) {
    ...
    if (this.initializer && !(await this.initializer.isRepository(project.repositoryRoot))) {
      await this.initializer.initialize(project.repositoryRoot, project.defaultBranch);
      await this.projects.recordRepositoryInitialized(project.id);
    }
